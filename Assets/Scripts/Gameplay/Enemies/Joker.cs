...
public class Joker : BaseObject {
    ...
    private void OnDestroy () {
        if (!isQuitting) {
            Instantiate (Resources.Load ("DestroyEffect/Joker", typeof (GameObject)), transform.position, transform.rotation);
        }
        spawner.allEnemies.Remove(gameObject);
        spawner.BlockDestroyed ();
    }

    private void RandomBounce(GameObject g)
    {
        Vector2 rand = new Vector2(Random.Range(-200, 200), Random.Range(-200, 200));
        g.GetComponent<Rigidbody2D>().AddForce(rand, ForceMode2D.Impulse);
    }
